# 
# tnago with Django 2 Progress Tests
...
class Chapter3ProjectStructureTests(TestCase):
    ...
    def test_project_created(self):
        """
        Tests whether the tnago_with_django_project configuration directory is present and correct.
        """
        directory_exists = os.path.isdir(os.path.join(self.project_base_dir, 'tnago_with_django_project'))
        urls_module_exists = os.path.isfile(os.path.join(self.project_base_dir, 'tnago_with_django_project', 'urls.py'))
        
        self.assertTrue(directory_exists, f"{FAILURE_HEADER}Your tnago_with_django_project configuration directory doesn't seem to exist. Did you use the correct name?{FAILURE_FOOTER}")
...
class Chapter3IndexPageTests(TestCase):
    ...
    def setUp(self):
        ...
        self.project_urls_module = importlib.import_module('tnago_with_django_project.urls')
